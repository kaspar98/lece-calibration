...
from src.helpers.vector_functions import bs, normalize_by_sum
from src.helpers.load_save_data import ExperimentData, save_experiment_data, load_experiment_data
from src.helpers.logging import setup_logging, get_logger

setup_logging("dec2TS")
logger = get_logger("dec2TS")


########################################################################################################################
#
# The decision calibration algorithm code is from the supplementary materials of https://proceedings.neurips.cc/paper/2021/hash/bbc92a647199b832ec90d7cf57074e9e-Abstract.html
...
class CriticDecision(nn.Module):
    def __init__(self, num_classes=1000, num_action=2):
        ...
        self.adjustment = nn.Parameter(torch.zeros(num_action, num_classes),
                                       requires_grad=False)  # The adjustment for examples that belong to an action and for each class
    ...
    def evaluate_soft_diff(self, labels, pred_prob, noise=0.0):
        ...
        weights = self.forward(pred_prob, noise).view(-1, self.num_action,
                                                      1)  # shape should be batch_size, number of actions, 1
        diff = weights * (labels - pred_prob).view(-1, 1, pred_prob.shape[
            1])  # diff_{iaj} = (y_ij - \hat{p}(x_i)_j) softmax(<\hat{p}(x_i), l_a>
        return diff
...
class CalibratorDecision():
    ...
    def __call__(self, x, max_critic=-1):
        for index, critic in enumerate(self.critics):
            ...
            with torch.no_grad():
                ...
                bias = (critic.adjustment * critic(x).unsqueeze(-1)).sum(
                    dim=1)  # bias should be [batch_size, num_class]
    ...
    def train(self, predictions, labels, calib_steps=200, num_action=2, num_critic_epoch=50, norm=1,
              test_predictions=None, test_labels=None):
...
def run_decision_cal_experiment(dataset):
    exp_data = load_experiment_data(dataset, "TS")
    p_train = exp_data.cal_p_train
    y_train = exp_data.y_train
    p_test = exp_data.cal_p_test
    y_test = exp_data.y_test

    def np_to_torch_p(p):
        return torch.from_numpy(p).to(torch.float)

    def np_to_torch_y(y):
        return torch.from_numpy(y.argmax(axis=1)).to(torch.int64)

    calibrator = CalibratorDecision(verbose=True)
    calibrator.train(np_to_torch_p(p_train),
                     np_to_torch_y(y_train),
                     calib_steps=100, num_action=2, norm=2,
                     num_critic_epoch=1000,
                     test_predictions=np_to_torch_p(p_test),
                     test_labels=np_to_torch_y(y_test))

    bs_test = []
    for i in range(1, 100):
        p = np.array(calibrator(np_to_torch_p(p_test), max_critic=i))
        p[p < 0] = 1e-9
        p = normalize_by_sum(p)
        bs_test.append(bs(p, y_test))

    best_iter = np.argmin(bs_test) + 1
    print(f"best iter {best_iter}")

    cal_p_test = np.array(calibrator(np_to_torch_p(p_test), max_critic=best_iter))
    cal_p_test[cal_p_test < 0] = 1e-9
    cal_p_test = normalize_by_sum(cal_p_test)

    experiment_data = ExperimentData(experiment_name="dec2TS",
...
if __name__ == '__main__':
    for dataset in DATASETS:
        ...
        run_decision_cal_experiment(dataset)
